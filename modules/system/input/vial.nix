...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    # Add udev rules for Vial keyboard access (Official Vial rules)
    services.udev.extraRules = ''
      # Official Universal Vial rule from get.vial.today
      KERNEL=="hidraw*", SUBSYSTEM=="hidraw", ATTRS{serial}=="*vial:f64c2b3c*", MODE="0660", GROUP="users", TAG+="uaccess", TAG+="udev-acl"

      # Kyria-specific vendor IDs (Pro Micro controllers)
      SUBSYSTEM=="usb", ATTRS{idVendor}=="1b4f", MODE="0660", GROUP="users", TAG+="uaccess"  # SparkFun Pro Micro
      SUBSYSTEM=="usb", ATTRS{idVendor}=="2341", MODE="0660", GROUP="users", TAG+="uaccess"  # Arduino
      SUBSYSTEM=="usb", ATTRS{idVendor}=="239a", MODE="0660", GROUP="users", TAG+="uaccess"  # Adafruit

      # Additional QMK/VIA/Vial vendor IDs
      SUBSYSTEM=="usb", ATTRS{idVendor}=="feed", MODE="0660", GROUP="users", TAG+="uaccess"  # QMK
      SUBSYSTEM=="usb", ATTRS{idVendor}=="1209", MODE="0660", GROUP="users", TAG+="uaccess"  # Generic QMK
      SUBSYSTEM=="usb", ATTRS{idVendor}=="16c0", MODE="0660", GROUP="users", TAG+="uaccess"  # VUSB
      SUBSYSTEM=="usb", ATTRS{idVendor}=="04d8", MODE="0660", GROUP="users", TAG+="uaccess"  # Microchip
